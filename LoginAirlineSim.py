...
def LoginAirlineSim(ServerName: str, UserName: str, Passwd: str) -> Session:
    ...
    login_session.get(login_fin_url, verify=local_Network_Debug, timeout=10000)  # 第一步，直接连接目标URL
    login_session.cookies['_sl_lp'] = quote(login_fin_url)
    login_session.headers['Referer'] = login_fin_url
    login_session.get('https://accounts.airlinesim.aero/auth/login?od=' + login_fin_url, verify=local_Network_Debug)
    # 构造提交数据，这里就不OPTIONS了
    ...
    login_result = login_session.post('https://sar.simulogics.games/api/sessions', json=post_json,
